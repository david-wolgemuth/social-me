...
class TabBarController2: UITabBarController {
    ...
    var FriendRequest: [String: String]?
    override func viewDidLoad() {
        super.viewDidLoad()
        
        self.tabBar.items![0].image = UIImage.fontAwesomeIconWithName(.Search, textColor: UIColor.blackColor(), size: CGSizeMake(30, 30))
        self.tabBar.items![1].image = UIImage.fontAwesomeIconWithName(.Users, textColor: UIColor.blackColor(), size: CGSizeMake(30, 30))
        let count = Connection.sharedInstance.getFriendRequestCount()
        if count > 0 {
            self.tabBar.items![1].badgeValue = String(Connection.sharedInstance.getFriendRequestCount())
